...
public class RIN_001 {
	...
	public void user_will_provide_all_the_inputs_select_the_record_on_the_grid() throws Exception {
		fleetpage.enterAccountNo(ExcelReader.FetchDataFromSheet(ConfigReader.readRINexcel(),"PreSetup",1,0));
		fleetpage.enterlastinactivedays(ExcelReader.FetchDataFromSheet(ConfigReader.readRINexcel(),"PreSetup",1,1));
	...
	public void user_will_navigate_to_distance_page_proceed() throws IOException, Exception {
		...
		//distancetabpage.validatesubhdr(ExcelReader.FetchDataFromSheet(ConfigReader.readRINexcel(),"PreSetup",1,2));
				commonobjects.clickProceed();
				Thread.sleep(1000);
				}
	...
	public void user_will_navigate_to_billing_page_provide_mandatory_inputs_proceed() throws IOException, Exception {
		...
		//billingtab.validatesubhdr(ExcelReader.FetchDataFromSheet(ConfigReader.readRINexcel(),"Billing",1,0));
		billingtab.enterreceiptdate(ExcelReader.FetchDataFromSheet(ConfigReader.readRINexcel(),"Billing",1,0));
				commonobjects.clickProceed();
				Thread.sleep(1000);
	}
	...
	public void user_will_validate_the_success_information_message() throws IOException, Exception {
		//Validating the information message
				commonobjects.validateInfoMessage(ExcelReader.FetchDataFromSheet(ConfigReader.readRINexcel(),"Billing",1,1));
				Thread.sleep(2000);
